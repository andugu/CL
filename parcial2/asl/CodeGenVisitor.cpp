...
antlrcpp::Any CodeGenVisitor::visitSumExpr(AslParser::SumExprContext *ctx){
  DEBUG_ENTER();
  ...
  if (ctx->expr().size() == 0){
    std::string temp = "%"+codeCounters.newTEMP();
    code = instruction::ILOAD(temp, std::to_string(0));
    addr = temp;
  }
    ...
    for (unsigned int i = 0; i < ctx->expr().size(); ++i){
        ...
        CodeAttribs codAts1 = visit(ctx->expr(i));
        std::string addr1 = codAts1.addr;
        instructionList code1 = codAts1.code;

      if (i == 0)
        code = code || code1 || instruction::ILOAD(temp, addr1);

      else {
        std::string temp2 = "%"+codeCounters.newTEMP();
        std::string no = "if"+codeCounters.newLabelIF();

        code = code || code1 || instruction::LT(temp2, temp, addr1) 
               || instruction::FJUMP(temp2, no)
               || instruction::ILOAD(temp, addr1) || instruction::LABEL(no);
      }
    }
    addr = temp;
  }
    ...
    for (unsigned int i = 0; i < ctx->expr().size(); ++i){
      ...
      if (i == 0){
        if (Types.isIntegerTy(t1))
          code = code || code1 || instruction::FLOAT(temp, addr1);
        else
          code = code || code1 || instruction::FLOAD(temp, addr1);
      }
        ...
        if (Types.isIntegerTy(t1))
          code = code || code1 || instruction::FLOAT(addr1, addr1) || instruction::FLT(temp2, temp, addr1) 
                 || instruction::FJUMP(temp2, no)
                 || instruction::FLOAD(temp, addr1) || instruction::LABEL(no);
        else
          code = code || code1 || instruction::FLT(temp2, temp, addr1) 
                 || instruction::FJUMP(temp2, no)
                 || instruction::FLOAD(temp, addr1) || instruction::LABEL(no);
      }
    }
