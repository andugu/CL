...
antlrcpp::Any TypeCheckVisitor::visitSumExpr(AslParser::SumExprContext *ctx) {
    DEBUG_ENTER();
    ...
    if (ctx->expr().size() > 0){
      ...
      for (auto i : ctx->expr()){
          ...
          else if (not Types.isNumericTy(t_param))
            not_numeric = true;

          else if (Types.isErrorTy(t_param))
              errors_exist = true;
      }

      if (not errors_exist and not not_numeric){
        if (exists_float)
          t = Types.createFloatTy();
        else
          t = Types.createIntegerTy();
